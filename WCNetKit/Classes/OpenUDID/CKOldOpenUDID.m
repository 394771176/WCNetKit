...
@interface CKOldOpenUDID (Private)
...
//
+ (NSString*) value {
    return [CKOldOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        NSString* slotPBid = [[NSString stringWithFormat:@"%@%d",kOpenUDIDSlotPBPrefix,n] retain];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [CKOldOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [CKOldOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [CKOldOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
